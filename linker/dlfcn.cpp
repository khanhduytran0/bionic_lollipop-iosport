...
#ifdef __APPLE__
#define __BIONIC_DLERROR_BUFFER_SIZE 1024
...
extern "C" void android_get_LD_LIBRARY_PATH(char* buffer, size_t buffer_size) {
...
void* dlsym(void* handle, const char* symbol) {
  ...
  if (sym != nullptr) {
    ...
    return nullptr;
#else
    void* darwinsym = dlsym_darwin(handle, symbol);
    if (darwinsym == NULL) {
      __bionic_format_dlerror("undefined symbol", symbol);
    }
    return darwinsym;
